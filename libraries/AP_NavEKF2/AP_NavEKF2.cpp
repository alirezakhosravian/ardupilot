...
const AP_Param::GroupInfo NavEKF2::var_info[] PROGMEM = {
    ...
    // @Range: 0.0001 0.01
    // @User: Advanced
    AP_GROUPINFO("MAGE_PNOISE",    12, NavEKF2, _magEarthProcessNoise, MAGE_PNOISE_DEFAULT),
    ...
    // @Range: 0.0001 0.01
    // @User: Advanced
    AP_GROUPINFO("MAGB_PNOISE",    13, NavEKF2, _magBodyProcessNoise, MAGB_PNOISE_DEFAULT),
...
//    AP_GROUPINFO("POS_DELAY",    15, NavEKF2, _msecPosDelay, 300),
    ...
    // @Increment: 50
    // @User: Advanced
    AP_GROUPINFO("GLITCH_ACCEL",    23, NavEKF2, _gpsGlitchAccelMax, GLITCH_ACCEL_DEFAULT),
...
bool NavEKF2::HeightDrifting(void) const
{
...
bool NavEKF2::PositionDrifting(void) const
{
...
void NavEKF2::ResetPosition(void)
{
    if (staticMode) {
        state.position.x = 0;
        state.position.y = 0;
...
void NavEKF2::ResetVelocity(void)
{
    if (staticMode) {
        state.velocity.zero();
        state.vel1.zero();
...
void NavEKF2::SelectVelPosFusion()
{
...
//
///*
//// calculate Delta matrix from quaternion form.
//D[0][0] = D_q[1]*D_q[1]+D_q[2]*D_q[2]-D_q[3]*D_q[3]-D_q[4]*D_q[4];
//D[0][1] = 2.0f*D_q[2]*D_q[3]-2.0f*D_q[1]*D_q[4];
//D[0][2] = 2.0f*D_q[1]*D_q[3]+2.0f*D_q[2]*D_q[4];
//D[1][0] = 2.0f*D_q[2]*D_q[3] + 2.0f*D_q[1]*D_q[4];
//D[1][1] = D_q[1]*D_q[1]-D_q[2]*D_q[2]+D_q[3]*D_q[3]-D_q[4]*D_q[4];
//D[1][2] = -2.0f*D_q[1]*D_q[2]+2.0f*D_q[3]*D_q[4];
//D[2][0] = -2.0f*D_q[1]*D_q[3] + 2.0f*D_q[2]*D_q[4];
//D[2][1] = 2.0f*D_q[1]*D_q[2] + 2.0f*D_q[3]*D_q[4];
//D[2][2] = D_q[1]*D_q[1]-D_q[2]*D_q[2]-D_q[3]*D_q[3]+D_q[4]*D_q[4];
//*/
//
//tilde_q = dAngIMU - state.gyro_bias;
//
...
//q_tmp[2] = state.quat[0]*q_hat[2] + state.quat[2]*q_hat[0] + state.quat[3]*q_hat[1] - state.quat[1]*q_hat[3];
//q_tmp[3] = state.quat[0]*q_hat[3] + state.quat[3]*q_hat[0] + state.quat[1]*q_hat[2] - state.quat[2]*q_hat[1];
//
//q_tmp.normalize();
//q_hat=q_tmp;
//
...
//q_tmp_m[0] = state.quat[0]*q_tmp[0] - state.quat[1]*q_tmp[1] - state.quat[2]*q_tmp[2] - state.quat[3]*q_tmp[3];
//q_tmp_m[1] = state.quat[0]*q_tmp[1] + state.quat[1]*q_tmp[0] + state.quat[2]*q_tmp[3] - state.quat[3]*q_tmp[2];
//q_tmp_m[2] = state.quat[0]*q_tmp[2] + state.quat[2]*q_tmp[0] + state.quat[3]*q_tmp[1] - state.quat[1]*q_tmp[3];
//q_tmp_m[3] = state.quat[0]*q_tmp[3] + state.quat[3]*q_tmp[0] + state.quat[1]*q_tmp[2] - state.quat[2]*q_tmp[1];
//
...
//*/
//
//// update predictor R_hat
///*
//R_hat_T = R_hat;
//R_hat[0][0] = D_T[0][0]*(D[0][0]*R_hat_T[0][0]+D[1][0]*R_hat_T[0][1]+D[2][0]*R_hat_T[0][2])+D_T[1][0]*(D[0][1]*R_hat_T[0][0]+D[1][1]*R_hat_T[0][1]+D[2][1]*R_hat_T[0][2])+D_T[2][0]*(D[0][2]*R_hat_T[0][0]+D[1][2]*R_hat_T[0][1]+D[2][2]*R_hat_T[0][2]);
//R_hat[0][1] = D_T[0][1]*(D[0][0]*R_hat_T[0][0]+D[1][0]*R_hat_T[0][1]+D[2][0]*R_hat_T[0][2])+D_T[1][1]*(D[0][1]*R_hat_T[0][0]+D[1][1]*R_hat_T[0][1]+D[2][1]*R_hat_T[0][2])+D_T[2][1]*(D[0][2]*R_hat_T[0][0]+D[1][2]*R_hat_T[0][1]+D[2][2]*R_hat_T[0][2]);
//R_hat[0][2] = D_T[0][2]*(D[0][0]*R_hat_T[0][0]+D[1][0]*R_hat_T[0][1]+D[2][0]*R_hat_T[0][2])+D_T[1][2]*(D[0][1]*R_hat_T[0][0]+D[1][1]*R_hat_T[0][1]+D[2][1]*R_hat_T[0][2])+D_T[2][2]*(D[0][2]*R_hat_T[0][0]+D[1][2]*R_hat_T[0][1]+D[2][2]*R_hat_T[0][2]);
//R_hat[1][0] = D_T[0][0]*(D[0][0]*R_hat_T[1][0]+D[1][0]*R_hat_T[1][1]+D[2][0]*R_hat_T[1][2])+D_T[1][0]*(D[0][1]*R_hat_T[1][0]+D[1][1]*R_hat_T[1][1]+D[2][1]*R_hat_T[1][2])+D_T[2][0]*(D[0][2]*R_hat_T[1][0]+D[1][2]*R_hat_T[1][1]+D[2][2]*R_hat_T[1][2]);
//R_hat[1][1] = D_T[0][1]*(D[0][0]*R_hat_T[1][0]+D[1][0]*R_hat_T[1][1]+D[2][0]*R_hat_T[1][2])+D_T[1][1]*(D[0][1]*R_hat_T[1][0]+D[1][1]*R_hat_T[1][1]+D[2][1]*R_hat_T[1][2])+D_T[2][1]*(D[0][2]*R_hat_T[1][0]+D[1][2]*R_hat_T[1][1]+D[2][2]*R_hat_T[1][2]);
//R_hat[1][2] = D_T[0][2]*(D[0][0]*R_hat_T[1][0]+D[1][0]*R_hat_T[1][1]+D[2][0]*R_hat_T[1][2])+D_T[1][2]*(D[0][1]*R_hat_T[1][0]+D[1][1]*R_hat_T[1][1]+D[2][1]*R_hat_T[1][2])+D_T[2][2]*(D[0][2]*R_hat_T[1][0]+D[1][2]*R_hat_T[1][1]+D[2][2]*R_hat_T[1][2]);
//R_hat[2][0] = D_T[0][0]*(D[0][0]*R_hat_T[2][0]+D[1][0]*R_hat_T[2][1]+D[2][0]*R_hat_T[2][2])+D_T[1][0]*(D[0][1]*R_hat_T[2][0]+D[1][1]*R_hat_T[2][1]+D[2][1]*R_hat_T[2][2])+D_T[2][0]*(D[0][2]*R_hat_T[2][0]+D[1][2]*R_hat_T[2][1]+D[2][2]*R_hat_T[2][2]);
//R_hat[2][1] = D_T[0][1]*(D[0][0]*R_hat_T[2][0]+D[1][0]*R_hat_T[2][1]+D[2][0]*R_hat_T[2][2])+D_T[1][1]*(D[0][1]*R_hat_T[2][0]+D[1][1]*R_hat_T[2][1]+D[2][1]*R_hat_T[2][2])+D_T[2][1]*(D[0][2]*R_hat_T[2][0]+D[1][2]*R_hat_T[2][1]+D[2][2]*R_hat_T[2][2]);
//R_hat[2][2] = D_T[0][2]*(D[0][0]*R_hat_T[2][0]+D[1][0]*R_hat_T[2][1]+D[2][0]*R_hat_T[2][2])+D_T[1][2]*(D[0][1]*R_hat_T[2][0]+D[1][1]*R_hat_T[2][1]+D[2][1]*R_hat_T[2][2])+D_T[2][2]*(D[0][2]*R_hat_T[2][0]+D[1][2]*R_hat_T[2][1]+D[2][2]*R_hat_T[2][2]);
//*/
...
void NavEKF2::CovariancePrediction()
{
...
void NavEKF2::FuseMagnetometer()
{
    ...
    uint16_t &obsIndex = mag_state.obsIndex;
...
void NavEKF2::zeroRows(Matrix22 &covMat, uint8_t first, uint8_t last)
{
    uint8_t row;
...
void NavEKF2::zeroCols(Matrix22 &covMat, uint8_t first, uint8_t last)
{
    uint8_t row;
...
void NavEKF2::quat2Tbn(Matrix3f &Tbn, const Quaternion &quat) const
{
    // Calculate the body to nav cosine matrix
...
void NavEKF2::SetFlightAndFusionModes()
{
...
void NavEKF2::CopyAndFixCovariances()
{
...
void NavEKF2::ConstrainVariances()
{
...
Quaternion NavEKF2::calcQuatAndFieldStates(float roll, float pitch)
{
...
bool NavEKF2::useAirspeed(void) const
{
 ...
 */
bool NavEKF2::assume_zero_sideslip(void) const
{
